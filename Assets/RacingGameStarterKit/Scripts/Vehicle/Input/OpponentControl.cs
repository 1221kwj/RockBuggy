...
namespace RGSK
{
    public class OpponentControl : MonoBehaviour
    {
        ...
        private VehicleController car_controller;
        ...
        [Range(0, 0.2f)]
        public float avoidanceSensitivity = 0.2f;
        ...
        void Awake()
        {
			...
			if (GetComponent<VehicleController>())
			{
				car_controller = GetComponent<VehicleController>();
				car_controller.brakes.Active = true;
			}
        ...
        void FixedUpdate()
        {
            ...
            if (car_controller)
            {
				currentSpeed = car_controller.Speed;

				topSpeed = 120;// (!brakezone && !stats.knockedOut) ? car_controller.Speed : brakeSpeed;
        ...
        void FeedInput(float motor, float brake, float steer)
        {
            ...
            {
				//motor value
				car_controller.input.Vertical = m;
				//car_controller.motorInput = m;

				//brake value
				//car_controller.input.Vertical += b;
				car_controller.input.Handbrake = b;
				...
				//steer value
				car_controller.input.Horizontal = s + avoidanceSteer;
        ...
        void RespawnRacer()
        {
            ...
            RaceManager.instance.RespawnRacer(transform, stats.lastPassedNode, 0.1f);
        }
