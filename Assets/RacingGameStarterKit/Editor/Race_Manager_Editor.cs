    ...
    public override void OnInspectorGUI()
    {
...
#if UNITY_EDITOR
		m_target.raceTime = (RaceManager.RaceTime)EditorGUILayout.EnumPopup("Race Time", m_target.raceTime);
#endif

		m_target.sun = EditorGUILayout.ObjectField("Sun", m_target.sun, typeof(Transform), true) as Transform;
		m_target.low = EditorGUILayout.ObjectField("Low Cloud", m_target.low, typeof(MeshRenderer), true) as MeshRenderer;
		m_target.high = EditorGUILayout.ObjectField("High Cloud", m_target.high, typeof(MeshRenderer), true) as MeshRenderer;
		EditorGUILayout.Space();
        ...
        switch (m_target._raceType)
        {
            ...
            */
            ...
            case RaceManager.RaceType.FuelLimit:
                //m_target.useFuel_Limit = EditorGUILayout.Toggle("useFuel", m_target.useFuel_Limit);
                m_target.capacity_Limit = EditorGUILayout.FloatField("Fuel Capacity", m_target.capacity_Limit);
                m_target.amount_Limit = EditorGUILayout.FloatField("Fuel Amount", m_target.amount_Limit);
                m_target.efficiency_Limit = EditorGUILayout.FloatField("Fuel Efficiency", m_target.efficiency_Limit);
                break;
        }
        ...
        //Default Fuel Value
        GUILayout.BeginVertical("Box");
        GUILayout.Box("Default Fuel Values", EditorStyles.boldLabel);
        EditorGUILayout.Space();

		bool check = m_target._raceType == RaceManager.RaceType.FuelLimit;

		if (check == true)
		{
			EditorGUI.BeginDisabledGroup(!check);
			{
				m_target.useFuel_Default = EditorGUILayout.Toggle("useFuel", m_target.useFuel_Default);
				m_target.capacity_Default = EditorGUILayout.FloatField("Fuel Capacity", m_target.capacity_Default);
				m_target.amount_Default = EditorGUILayout.FloatField("Fuel Amount", m_target.amount_Default);
				m_target.efficiency_Default = EditorGUILayout.FloatField("Fuel Efficiency", m_target.efficiency_Default);
			}
			EditorGUI.EndDisabledGroup();
		}
		else
			m_target.useFuel_Default = false;
